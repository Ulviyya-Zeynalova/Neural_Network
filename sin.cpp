...
void gen(int n0, int n1, double h, double w, double a) {
    ...
    random_device rd;
    mt19937 gen(rd());
    uniform_real_distribution<double> dist_x(-2.0, 2.0);
    uniform_real_distribution<double> dist_y(-2.0, 2.0);
    int count=0;
    while (count<n0) {
        double x=dist_x(gen);
        double y=dist_y(gen);
        double upper=h*sin(w*x)+a/2.0;
        double lower=h*sin(w*x)-a/2.0;
        if (y>lower && y<upper) {
            f<<x<<" "<<y<<" 0"<<endl;
            count++;
			}
